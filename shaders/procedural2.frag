...
#define Mouse (vec2(0.0,0.0) / iResolution.xy)
...
float getwaves(vec2 position, int iterations){
    ...
    for(int i=0;i<iterations;i++){
        ...
        vec2 res = wavedx(position, p, speed, phase, delta_time);
...
float H = 0.0;
vec3 norm(vec2 pos, float e, float depth){
    vec2 ex = vec2(e, 0);
...
void main()
{
    ...
    vec2 uv = (pos.xz + vec2(15, 7.5)) / vec2(30, 15);
    ...
    vec3 N = norm(pos.xz, 0.001, waterdepth);
